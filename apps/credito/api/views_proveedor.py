...
from apps.credito.models import (
    CreditoProveedor,
    OperacionPrelacionPago,
...
class PagosCreditoProveedorViewSet(viewsets.ModelViewSet):
    ...
    @staticmethod
    def _build_request_hash(proveedor_id, cantidad_pagar, pool):
        canonical = {
            'tipo': 'PROVEEDOR',
    ...
    def prelacion(self, request):
        ...
        if not proveedor_id:
            return self._error_response(
                'Debe enviar el campo proveedor.',
                'MISSING_PROVEEDOR',
        ...
        try:
            ...
            return self._error_response(
                'El campo proveedor debe ser un entero válido.',
                'INVALID_PROVEEDOR_ID',
                status.HTTP_400_BAD_REQUEST,
            )
        ...
        request_hash = self._build_request_hash(proveedor_id_int, cantidad_pagar, pool)
        self._log_prelacion_event(
            request,
            event='REQUEST_RECEIVED',
            entidad_id=proveedor_id_int,
            idempotency_key=idempotency_key,
        ...
        with transaction.atomic():
            operacion = (
                ...
                .filter(
                    tipo=OperacionPrelacionPago.TIPO_PROVEEDOR,
                    entidad_id=proveedor_id_int,
                    idempotency_key=idempotency_key,
            ...
            if operacion:
                if operacion.request_hash != request_hash:
                    detail = (
                        ...
                        f'para proveedor {proveedor_id_int}.'
                    )
                    self._log_prelacion_event(
                        request,
        ...
        try:
            with transaction.atomic():
                ...
                creditos = list(
                    CreditoProveedor.objects.select_for_update().filter(
                ...
                if not creditos:
                    raise ValueError('El proveedor no tiene créditos activos para aplicar prelación.')
